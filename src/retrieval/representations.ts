...
function splitLongText(text: string): readonly string[] { if (text.length <= ARTICLE_SECTION_BODY_LIMIT) return [text]; const result: string[] = []; let remaining = text; while (remaining.length > ARTICLE_SECTION_BODY_LIMIT) { const boundary = remaining.lastIndexOf(" ", ARTICLE_SECTION_BODY_LIMIT); const fallback = remaining.charCodeAt(ARTICLE_SECTION_BODY_LIMIT - 1); const index = boundary > 0 ? boundary : (fallback >= 0xd800 && fallback <= 0xdbff && remaining.charCodeAt(ARTICLE_SECTION_BODY_LIMIT) >= 0xdc00 && remaining.charCodeAt(ARTICLE_SECTION_BODY_LIMIT) <= 0xdfff ? ARTICLE_SECTION_BODY_LIMIT - 1 : ARTICLE_SECTION_BODY_LIMIT); result.push(remaining.slice(0, index).trim()); remaining = remaining.slice(index).trim(); } if (remaining) result.push(remaining); return result; }
function splitParagraphs(text: string): readonly string[] { const chunks: string[] = []; let pending = ""; for (const paragraph of text.split(/\n[\t ]*\n/)) { if (!paragraph.trim()) continue; const combined = pending ? `${pending}\n\n${paragraph}` : paragraph; if (combined.length <= ARTICLE_SECTION_BODY_LIMIT) pending = combined; else { if (pending) chunks.push(pending); pending = ""; if (paragraph.length <= ARTICLE_SECTION_BODY_LIMIT) pending = paragraph; else chunks.push(...splitLongText(paragraph)); } } if (pending) chunks.push(pending); return chunks; }
function splitBody(body: string): readonly { heading?: string; text: string }[] { const sections: { heading?: string; text: string }[] = []; let heading: string | undefined; let current: string[] = []; const flush = () => { const text = normalizeText(current.join("\n")); if (text) sections.push({ heading, text }); current = []; }; for (const line of normalizeText(body).split("\n")) { const match = /^(#{1,6})\s+(.+?)\s*#*\s*$/.exec(line); if (match) { flush(); heading = normalizeText(match[2]!); } else current.push(line); } flush(); return sections.flatMap((section) => splitParagraphs(section.text).map((text) => ({ heading: section.heading, text }))); }
