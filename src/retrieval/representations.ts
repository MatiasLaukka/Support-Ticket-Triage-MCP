...
const unsafeCaseText = /(?:\b(?:raw\s+)?(?:system|developer|user)\s+(?:prompt|message|instructions?)\b|\braw\s+prompt\b|\b(?:hidden|chain[- ]of[- ]thought|reasoning)\b|\b(?:api[-_]?key|access[-_]?token|secret|password)\s*[=:]\s*\S+|\bsk-[a-z0-9_-]+\b|\b(?:[a-z]:[\\/]|\\\\)|(?:^|\s)[~\/][^\s]*|\b(?:model|provider)\s+(?:payload|response)\b|^\s*[\[{]|https?:\/\/[^\s/@]+:[^\s/@]+)/i;
export function safeCaseText(text: string, identifiers: readonly string[]): string | undefined { let value = normalizeText(text); for (const identifier of identifiers) value = value.replaceAll(identifier, ""); value = normalizeText(value.replace(/\s{2,}/g, " ")); return value.length > 0 && !unsafeCaseText.test(value) ? value : undefined; }
